  ...
  <a-drawer :visible="isShow" :title=" isAdd ? '新增角色' : '修改角色' " width="600" :maskClosable="false" @close="isShow = false">
    ...
    <a-form-model ref="infoFormModel" :model="saveObject" :label-col="{span: 4}" :rules="rules">
      ...
      <a-form-model-item label="角色标识：" prop="roleCode">
        <a-input v-model="saveObject.roleCode" />
      </a-form-model-item>
      <a-form-model-item label="角色描述：" prop="roleDesc">
        <a-input v-model="saveObject.roleDesc" />
      </a-form-model-item>
...
export default {
  ...
  methods: {
    show: function (recordId) { // 弹层打开事件
      ...
      // 初始化角色权限分配功能
      this.$nextTick(() => that.$refs.roleDist.initTree(recordId, this.isAdd))
    ...
    handleOkFunc: function () { // 点击【确认】按钮事件
        ...
        this.$refs.infoFormModel.validate(valid => {
          if (valid) { // 验证通过
            ...
            that.saveObject.authorityIds = selectedEntIdList || ''
