...
<script>
import { getEntTree, API_URL_ROLE_AUTH_RELA_LIST, req } from '@/api/manage'
export default {

  data () {
    return {
      hasEnt: this.$access('role_edit'),
      recordId: null, // 更新对象ID
      treeData: [],
      replaceFields: { key: 'id', title: 'name' }, // 配置替换字段
      checkedKeys: [], // 已选中的节点
  ...
  methods: {
    ...
    initTree: function (recordId, isAdd) { // 弹层打开事件
      const that = this
      ...
      getEntTree().then(res => {
        ...
        this.recursionTreeData(res, (item) => {
          that.allEntList[item.id] = { pid: item.pid, children: item.children || [] }
        })

        // 查询所有的已分配的权限集合 (默认为 0 ， 无数据)
        if (!isAdd) {
          req.getById(API_URL_ROLE_AUTH_RELA_LIST, recordId).then(res2 => {
            const checkedEntIdList = [] // 所有已分配的权限集合（兼容antd vue 仅保留子节点）

            if (res2) {
              res2.map(item => {
                if (that.allEntList[item.authId] && that.allEntList[item.authId].children.length <= 0) { // 说明是子节点
                  checkedEntIdList.push(item.authId)
                }
              })
