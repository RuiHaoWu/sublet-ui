  ...
  <page-header-wrapper>
    <a-card>
      <div class="table-page-search-wrapper">
        <a-form layout="inline" class="table-head-ground">
          <div class="table-layer">
            <jeepay-text-up :placeholder="'角色标识'" :msg="searchData.roleCode" v-model="searchData.roleCode" />
        ...
        <div>
          <a-button v-if="$access('role_add')" type="primary" icon="plus" @click="addFunc" class="mg-b-30">新建</a-button>
        </div>
      </div>
      ...
      <JeepayTable
        ...
        rowKey="roleName"
      >
        <template slot="roleIdSlot" slot-scope="{record}"><b>{{ record.roleCode }}</b></template> <!-- 自定义插槽 -->
        <template slot="opSlot" slot-scope="{record}">  <!-- 操作列插槽 -->
          <JeepayTableColumns>
            <a v-if="$access('role_edit')" @click="editFunc(record.roleId)">修改</a>
            <a style="color: red" v-if="$access('role_del')" @click="delFunc(record.roleId)">删除</a>
          </JeepayTableColumns>
        </template>
      </JeepayTable>
    </a-card>
...
const tableColumns = [
  {
    key: 'roleCode', // key为必填项，用于标志该列的唯一
    title: '角色标识',
    sorter: true,
    fixed: 'left',
