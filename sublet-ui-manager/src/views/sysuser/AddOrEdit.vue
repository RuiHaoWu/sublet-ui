      ...
      <a-row justify="space-between" type="flex">
        <a-col :span="10">
          <a-form-model-item label="用户名:" prop="username">
            <a-input v-model="saveObject.username" :disabled="!isAdd" />
          </a-form-model-item>
        </a-col>
        ...
        <a-col :span="10" v-if="isAdd">
          <a-form-model-item label="密码：" prop="password">
            <a-input v-model="saveObject.password" />
          </a-form-model-item>
        </a-col>
...
import AvatarModal from '../current/AvatarModal'
export default {
    components: {
      AvatarModal,
      JeepayUpload
    ...
    return {
      ...
      rules: {
        ...
        confirmPwd: [{ required: false, trigger: 'blur' }, {
          validator: (rule, value, callBack) => {
            if (!this.sysPassword.defaultPass) {
              this.newPwd === this.sysPassword.password ? callBack() : callBack('新密码与确认密码不一致')
            } else {
              callBack()
            }
          }
      ...
      this.$refs.infoFormModel.validate(valid => {
        if (valid) { // 验证通过
          ...
          if (that.isAdd) {
            ...
            that.sysPassword.password = Base64.encode(that.sysPassword.password)
