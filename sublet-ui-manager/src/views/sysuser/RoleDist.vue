...
<script>
import { assignRole, req, reqLoad, API_URL_ROLE_LIST, API_URL_USER_ROLE_RELA_LIST } from '@/api/manage'
export default {
  ...
  methods: {
    ...
    show: function (recordId) { // 弹层打开事件
      ...
      reqLoad.list(API_URL_ROLE_LIST, { pageSize: -1 }).then(res => {
        ...
        // 查询已分配的列表
        req.getById(API_URL_USER_ROLE_RELA_LIST, recordId).then(relaRes => {
          relaRes.map(rela => {
            console.log(rela + '1')
    ...
    handleOkFunc: function () { // 点击【确认】按钮事件
      ...
      assignRole(this.recordId, this.checkedVal).then(res => {
